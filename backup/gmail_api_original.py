...
# Use more permissive scope for testing
...
class GmailAPI:
    ...
    def search_emails(self, query_terms, max_results=50):
        search_query = ' '.join(query_terms)
        messages = self.get_messages(query=search_query, max_results=max_results)
    ...
    def get_job_related_emails(self, days=30, max_results=100):
        job_keywords = [
            'interview', 'application', 'position', 'job', 'hiring',
            'recruiter', 'hr', 'offer', 'candidate', 'resume'
        ]
